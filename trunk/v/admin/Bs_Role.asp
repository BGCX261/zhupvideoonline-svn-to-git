...
<div id="sysMain">
<div id="sysNavPath">您的位置：后台用户角色列表页</div>
<div id="sysSch"><span>后台用户角色列表</span>
...
%>
<img border=0 src=../images/delete.gif alt="删除此角色" style="cursor:hand" onClick="{if(confirm('该操作不可恢复！\n\n确实要删除这个角色吗？ ')){location.href='Bs_Role.asp?deladm=<%=rs("id")%>';}}"> <%=rs("roleContent")%></div>
	<%
	rs.movenext
	loop
	%>
...
<form action="Bs_Role.asp" method=post name=manage onSubmit="return chk();">
	...
	Set rs = conn.Execute("select * from Bs_Role where id=CInt('"&roleId&"')")
		  if rs.eof and rs.bof then
 		  ...
 		  manage=rs("rolePower")
		%>
      <font color=red><b><%=rs("roleName")%></b></font> 角色的管理权限：
  <div>
  角色名称：<input name="roleNameNew" type="text" value="<%=rs("roleName")%>" maxlength="20">
            <input name="roleNameOld" type="hidden" value="<%=rs("roleName")%>">
  角色说明：<input name="roleContent" type="text" value="<%=rs("roleContent")%>" maxlength="50">
  </div>
...
新增</td>
    ...
    <tr class="sysListSingleTr">
      ...
      <div class="sysSchSmtTd"><input type="submit" name="Submit" value=" 保 存 ">
		...
		<input type="hidden" name="roleId" value="<%=roleId%>"> <input name="input" type="button" value=" 返 回 " onClick="window.history.go(-1)" />
      </div>
</form>
<%   
else
...
sub deladmin()
	...
	rs.open sql,conn,1,3
		if (rs.bof and rs.eof) then
			response.write "<script language='javascript'>"
			response.write "alert('操作失败，没有此角色！');"
			response.write "location.href='Bs_Role.asp?roleId="&roleId&"';"
			response.write "</script>"
			response.end
		elseif deladm=1 then
			response.write "<script language='javascript'>"
			response.write "alert('您不能删除这条！');"
			response.write "location.href='Bs_Role.asp?roleId="&roleId&"';"
			response.write "</script>"
			response.end
		else
