...
<div class="img_sheet">
  ...
  <div class="unit"><div class="in">
    <table cellspacing="0" cellpadding="0">
      ...
      <tr><td class="title">
        <a href="{$S_ROOT}?product_{$product[$i]['art_id']}.{$suffix}" target="_blank">{$left($product[$i]['art_title'],11)}</a>
      </td></tr>
    </table>
  </div></div>
  {/for}
...
<div id="picture">
  ...
  <div id="prev_next">
    <div class="prev">{$L_PREV}：{if:$prev_title,neq,""}<a href="{$S_ROOT}?product_{$prev_id}.html">{$left($prev_title,20)}</a>{else}{$L_NONE}{/if}</div>
    <div class="next">{$L_NEXT}：{if:$next_title,neq,""}<a href="{$S_ROOT}?product_{$next_id}.html">{$left($next_title,20)}</a>{else}{$L_NONE}{/if}</div>
    <div class="clear"></div>
  </div>
</div>
{/if}
