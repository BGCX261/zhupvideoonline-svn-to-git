...
<div class="art_sheet list">
  <ul class="main">
    {for:$i,0,$article_u}
    <li><a href="{$S_ROOT}?recruit_{$article[$i]['art_id']}.{$suffix}" target="_blank">{$cut_str($article[$i]['art_title'],50)}</a><span>{$datevalue($article[$i]['art_add_time'])}</span><div class="clear"></div></li>
    <!-- 正文截取
	{$cut_str(remove_html($article[$i]['art_text']),200)}
	<a href="{$S_ROOT}?recruit_{$article[$i]['art_id']}.{$suffix}" target="_blank">【更多】</a>
	-->
	{/for}
  </ul>
...
<div class="page_link"><div class="in">
  ...
  {if:$page_sum,neq,1}
    ...
    <a class="num" href="{$S_ROOT}?recruit_{$cat}_{$page}_0.{$suffix}" style="color:#2874c2;">【{$page}】</a>
...
<div id="article">
  ...
  <div id="prev_next">
    <div class="prev">{$L_PREV}：{if:$prev_title,neq,""}<a href="{$S_ROOT}?recruit_{$prev_id}.html">{$cut_str($prev_title,20)}</a>{else}{$L_NONE}{/if}</div>
    <div class="next">{$L_NEXT}：{if:$next_title,neq,""}<a href="{$S_ROOT}?recruit_{$next_id}.html">{$cut_str($next_title,20)}</a>{else}{$L_NONE}{/if}</div>
  </div>
</div>
{/if}
<!-- 新秀 -->
