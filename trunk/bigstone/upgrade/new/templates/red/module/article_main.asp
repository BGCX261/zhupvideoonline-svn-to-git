<%response.end%>
{if:$show_sheet,eq,1}
<div class="art_sheet list2">
  <ul class="main">
    {for:$i,0,$article_u}
    <li><a href="{$S_ROOT}?article_{$article[$i]['art_id']}.{$suffix}" target="_blank">{$cut_str($article[$i]['art_title'],50)}</a><span>{$datevalue($article[$i]['art_add_time'])}</span><div class="clear"></div></li>
    {/for}
  </ul>
...
<div class="page_link"><div class="in">
  ...
  {if:$page_sum,neq,1}
    ...
    <a class="num" href="{$S_ROOT}?article_{$cat}_{$page}_0.{$suffix}" style="color:#d90000;">【{$page}】</a>
...
<div id="article">
  ...
  <div id="prev_next">
    <div class="prev">{$L_PREV}：{if:$prev_title,neq,""}<a href="{$S_ROOT}?article_{$prev_id}.html">{$cut_str($prev_title,20)}</a>{else}{$L_NONE}{/if}</div>
    <div class="next">{$L_NEXT}：{if:$next_title,neq,""}<a href="{$S_ROOT}?article_{$next_id}.html">{$cut_str($next_title,20)}</a>{else}{$L_NONE}{/if}</div>
  </div>
</div>
{/if}
<!-- 新秀 -->
