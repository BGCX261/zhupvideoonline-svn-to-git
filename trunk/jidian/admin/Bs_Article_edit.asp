...
%>								</td>
							...
							<tr> 
								...
								<span class="red">*</span> 作者<input name="author" type="text" id="author" value='<%if session("realname") then response.write(session("realname")) else response.write ("请输入")end if%>' readonly="readonly" /></td>
							</tr>
							<tr> 
...
%>
							<tr> 
								...
								<span class="red">*</span><br>用来查找相关文章，可输入多个关键字，中间用空格分开。不能出现&quot;'*?()等字符。</td>
							</tr>
							<tr> 
...
%>
							...
							<tr> 
                                ...
                                <input name="Passed" type="checkbox" id="Passed" value="yes" <% if instr(manage,str)<=0 then response.Write("") else if rsSysAdmin("Passed")=true then response.Write("checked") end if%> <%call checkverify("53")%>>
							...
							<tr> 
								...
								<input type="hidden" name='bigclass'>
				                  <input name="Save" type="submit"  id="Save" value="保存修改结果"></td>
							</tr>
						</table>
			</form>
		</td>
