...
rs.open "Select * From innovType",conn,1,3

if Request.QueryString("no")="yes" then
frtype= Trim(Request("frtype"))
Brief= Trim(Request("Brief"))
if frtype<>"" then
sql="delete from innovType where frtype='" & frtype & "'"
conn.Execute  sql
sql="delete from innovation where frtype='" & frtype & "'"
conn.Execute sql
end if
...
<table align="center" class="a2">
	...
	<tr class="a4">
		<td>
          <form method="post" action="Bs_innovType.asp?no=eshop">
			<table width="100%">
                <tr> 
            ...
            <table width="100%">
                ...
                <tr class="opt"> 
                  <td><%=rs("frtype")%></td>
                  <td><a href="Bs_innovType_edit.asp?id=<%=rs("id")%>">修改</a></td>
                  <td><a href="Bs_innovType.asp?frtype=<%=rs("frtype")%>&amp;no=yes">删除</a></td>
                </tr>
<%
rs.MoveNext 
loop 
