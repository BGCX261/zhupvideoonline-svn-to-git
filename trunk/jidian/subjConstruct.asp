...
if nbtyped="" then                      
    sql="select * from SubjConstruct order by id desc"                      
else                      
    sql="select * from SubjConstruct where nbtyped='"&request("nbtyped")&"' order by id desc"                    
end if 
...
rs.open sql,conn,1,1
	...
	response.write  "<div class='artpath'><img src='img/titleRed_ico.gif' /> <a href='SubjConstruct.asp'>党建工作</a> >> "
	if nbtyped="" then
		response.write "所有信息"
	else
		if nbtyped<>"" then
			response.write "<a href='SubjConstruct.asp?nbtyped=" & nbtyped & "'>" & nbtyped & "</a> "
		end if
	end if
    response.write  "</div>"
...
if rs.eof and rs.bof then                      
      ...
      if (currentpage-1)*MaxPerPage>totalput then                      
       ...
       if currentPage=1 then                                      
           showContentS                      
           showpage totalput,MaxPerPage,"SubjConstruct.asp"           
       else                      
          if (currentPage-1)*MaxPerPage<totalPut then                      
   ...
   i=0 %>
                <ul>
				  ...
				  <li  <%if rs("toTop")=true then response.write "class='toTop'" else response.write "class=''" end if%>>
					 ...
					 <span class="artlistupdate"><%=formatdatetime(rs("crt_time"),2)%></span>
                  </li>
<% 
