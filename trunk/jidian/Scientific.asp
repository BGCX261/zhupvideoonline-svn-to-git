...
if nbtyped="" then                      
    sql="select * from Scientific order by id desc"                      
else                      
    sql="select * from Scientific where nbtyped='"&request("nbtyped")&"' order by id desc"                    
end if 
...
rs.open sql,conn,1,1
	...
	response.write  "<div class='artpath'><img src='img/title_ico.gif' /> <a href='Scientific.asp'>教学科研</a> >> "
	if nbtyped="" then
		response.write "所有信息"
	else
		if nbtyped<>"" then
			response.write "<a href='Scientific.asp?nbtyped=" & nbtyped & "'>" & nbtyped & "</a> "
		end if
	end if
    response.write  "</div>"
...
if rs.eof and rs.bof then                      
      ...
      if (currentpage-1)*MaxPerPage>totalput then                      
       ...
       if currentPage=1 then                                      
           showContent                      
           showpage totalput,MaxPerPage,"Scientific.asp"           
       else                      
          if (currentPage-1)*MaxPerPage<totalPut then                      
...
<% sub showContent                      
   ...
   i=0 %>
                <ul>
                  ...
                  i=i+1%>
				  <li>
                     <span class="artlnk"><a href="ScientificInfo.asp?id=<%=rs("id")%>"><%=cutstr(rs("Title"),50)%></a></span>
