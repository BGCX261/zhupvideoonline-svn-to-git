...
<!-- #include file="Inc/Head.asp" -->
  <div id="body">
    <table class="bodytable">
      <tr>
        <td class="bodyleft">
            <!-- #include file="L_facResource.asp" -->
        </td>
...
if frtype="" then                      
    sql="select * from FacResource order by id desc"                      
else                      
    sql="select * from FacResource where frtype='"&request("frtype")&"' order by id desc"                    
end if 
...
rs.open sql,conn,1,1
	...
	response.write  "<div class='artpath'><img src='img/title_ico.gif' /> <a href='facResource.asp'>实践教学</a> >> "
	if frtype="" then
		response.write "所有信息"
	else
		if frtype<>"" then
			response.write "<a href='facResource.asp?frtype=" & frtype & "'>" & frtype & "</a> "
		end if
	end if
    response.write  "</div>"
...
if rs.eof and rs.bof then                      
      ...
      if (currentpage-1)*MaxPerPage>totalput then                      
       ...
       if currentPage=1 then                                      
           showContent                      
           showpage totalput,MaxPerPage,"facResource.asp"           
       else                      
          if (currentPage-1)*MaxPerPage<totalPut then                      
...
<% sub showContent                      
   ...
   i=0 %>
                <ul>
                  ...
                  i=i+1%>
				  <li>
                        <span class="artlnk"><a href="facResourceInfo.asp?id=<%=rs("id")%>"><%=cutstr(rs("Title"),100)%></a></span>
