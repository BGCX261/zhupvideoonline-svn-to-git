...
sub showpage(sfilename,totalnumber,maxperpage,ShowTotal,ShowAllPages,strUnit)
	...
	if totalnumber mod maxperpage=0 then
  	...
  	strTemp= "<form name='showpages' method='Post' action='" & sfilename & "'><img src='img/arrow.gif' />"
	if ShowTotal=true then 
		strTemp=strTemp & "共 <span id='numnews'>" & totalnumber & "</span> " & strUnit & "&nbsp;&nbsp;"
	end if
	strUrl=JoinChar(sfilename)
  	if CurrentPage<2 then
    		strTemp=strTemp & "<span id='gray'>首页 上一页</span>"
  	else
    		strTemp=strTemp & "<span id='light'><a href='" & strUrl & "page=1'>首页</a></span>"
    		strTemp=strTemp & "<span id='light'><a href='" & strUrl & "page=" & (CurrentPage-1) & "'>上一页</a></span>"
  	end if

  	if n-currentpage<1 then
    		strTemp=strTemp & "<span id='gray'>下一页 尾页</span>"
  	else
    		strTemp=strTemp & "<span id='light'><a href='" & strUrl & "page=" & (CurrentPage+1) & "'>下一页</a></span>"
    		strTemp=strTemp & "<span id='light'><a href='" & strUrl & "page=" & n & "'>尾页</a></span>"
  	end if
   	strTemp=strTemp & "&nbsp;页次：<span id='numnews'>" & CurrentPage & "</span>/" & n & "页 "
    strTemp=strTemp & "&nbsp;<span id='numnews'>" & maxperpage & "</span>" & strUnit & "/页"
